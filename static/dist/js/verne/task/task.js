...
//Recibe el id del div, si el boton contiene un evento lo deshabilite
...
//El numero indica la cantidad de lugares que debe correr entre cada slash
console.log("url.searchParams.get('id') ",url_string.split('/')[5])
console.log("url.searchParams.get('fase') ",url_string.split('/')[6])
let id_idea_url = url_string.split('/')[5]
let id_fase_url = url_string.split('/')[6]
...
function abrir_modal_creacion(url) {
    ...
    $.ajax({
        ...
        complete: function (res) {
            $("#loader").hide();
            //Deshabilita el boton que abre el modal de crear
            activarBotonModalCRUD('#abrir_modal_creacion')
            $('#fase_form_create1').appendTo($('#fase_form_create1').val(id_idea_url))
            $('#id_form_create1').appendTo($('#id_form_create1').val(id_fase_url))
            // Appendto permite mover un elemento de un alado a otro del dom, aqui movemos el id idea al campo oculto
        },
...
//Valida si existe el botón de creación, y envia la url al hacer click
if (document.getElementById("abrir_modal_creacion")) {
    document.getElementById("abrir_modal_creacion").addEventListener("click", function () {
        abrir_modal_creacion('/ideas/task_create/');
    });
}
        ...
        error: function (xhr, ajaxOptions, thrownError) {
            ...
            try {
                ...
                if (xhr.status == 403) {
                    ...
                    Swal.fire({
                        title: 'Error!' + thrownError,
                        text: 'No cuentas con el permiso necesario para acceder',
                        icon: 'error',
                        allowOutsideClick: false,
                    })
                }
    ...
    let table = $('#tabla_listar').DataTable({
        ...
        "columns": [
...

            {
                ...
                'width': '30%',
                render: function (data, type, row) {
                    return '<div class="row justify-content-center"><div class="col-6 col-xl-6 col-lg-6 col-sm-6 text-right"><button id="editar_lista_' + row.task__id + '"' + 'type="button" class="btn btn-info " onclick="abrir_modal_edicion(\'/ideas/task_update/' + row.task__id + '/\');"><i class="fa fa-edit fa-1x"></i></button></div>' + '<div class="col-6 col-xl-6 col-lg-6 col-sm-6 text-left"><button id="eliminar_lista_' + row.task__id + '"' + 'type="button" class="btn btn-danger" onclick="abrir_modal_eliminacion(\'/ideas/task_delete/' + row.task__id + '/\');"><i class="fas fa-trash-alt fa-1x"></button></div></div>';
                    //return 'hola'
                },
